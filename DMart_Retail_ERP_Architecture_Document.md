# DMart Retail ERP – Technical Architecture & Implementation Blueprint

> **System Scope:** Large-Scale Enterprise Retail ERP (DMart-Style Supermarket & E-Commerce System)  
> **Target Architecture:** Distributed Microservices + CQRS + Event-Driven Architecture  
> **Technology Stack:** ASP.NET Core (.NET 8/9), Angular Frontend, YARP API Gateway, SQL Server, MongoDB, RabbitMQ, Redis, gRPC  

---

## 1. Executive Summary & System Vision

This document defines the production-grade, enterprise architectural specification for building a full-scale retail ERP platform modelled after **DMart**. The system manages end-to-end supermarket operations, including store management, warehouse operations, central procurement, POS cashier counter billing, online order fulfillment, multi-tier pricing/promotions, financial ledger accounting, and real-time inventory management.
...
The architecture comprises **14 core business microservices** plus **infrastructure services**.

| # | Microservice | Primary Responsibility | Write Storage (SQL Server) | Read Storage (MongoDB / Redis) |
|---|---|---|---|---|
| 1 | **Identity Service** | AuthN/AuthN, Users, Roles, JWT/OAuth2, Security Audit | `Db_Identity` | Redis (Token Blacklist & Sessions) |
| 2 | **Product Service** | Product Master, SKU, Barcodes, Categories, Tax Slabs | `Db_Product` | `Mongo_ProductRead` |
| 3 | **Store Service** | Physical Stores, POS Counters, Store Registers & Terminals | `Db_Store` | `Mongo_StoreRead` |
| 4 | **Inventory Service** | Real-time Stock, Transactions, Adjustments, Transfers | `Db_Inventory` | `Mongo_InventoryRead` |
| 5 | **Warehouse Service** | Racks, Bins, Goods Receiving (GRN), Picking, Packing | `Db_Warehouse` | `Mongo_WarehouseRead` |
| 6 | **Purchase Service** | Supplier Management, Purchase Orders (PO), Approvals | `Db_Purchase` | `Mongo_PurchaseRead` |
| 7 | **Sales / POS Service**| Retail Counter Cash Billing, Invoicing, Counter Returns | `Db_Sales` | `Mongo_SalesRead` |
| 8 | **Order Service** | E-commerce / Home Delivery Orders & Fulfillment Cycle | `Db_Order` | `Mongo_OrderRead` |
| 9 | **Customer Service** | Customer Profiles, Addresses, Loyalty Points & Tiers | `Db_Customer` | `Mongo_CustomerRead` |
| 10 | **Pricing Service** | Base Price, Store-Level Prices, Schemes, Bulk Offers | `Db_Pricing` | Redis (Cached Price Lists) |
| 11 | **Payment Service** | Payment Gateway Integration, Cash/Card/UPI, Refunds | `Db_Payment` | `Mongo_PaymentRead` |
| 12 | **Finance Service** | General Ledger, Expense Tracking, Supplier Settlements | `Db_Finance` | `Mongo_FinanceRead` |
| 13 | **Notification Service**| SMS, Email, Push Notifications, WhatsApp Alerts | `Db_Notification` | MongoDB (Notification History) |
...
### 4.1 Product Service
* **Responsibility:** Manages the centralized product catalog (Product Master).
...
* **Responsibility:** Tracks stock quantities, batch numbers, expiry dates, and movement audit logs across warehouses and physical stores.
* **Entities & Relational Tables:**
...
* **Responsibility:** Serves billing counters at physical supermarkets. Must support fast barcode scanning, bill generation, and cash/UPI receipt printing under tight latency constraints.
* **Entities & Relational Tables:**
...
* **Responsibility:** Manages vendor relationships, purchase requisitions, PO approval workflows, and Goods Received Notes (GRN).
* **Entities & Relational Tables:**
...
The platform implements CQRS using **MediatR** in .NET Core to isolate Write (Command) operations from Read (Query) operations.
...
Every microservice repository follows .NET Clean Architecture principles:

```text
RetailERP/
├── src/
│   ├── BuildingBlocks/
│   │   ├── ERP.BuildingBlocks.CQRS/          # MediatR interfaces & behaviors
│   │   ├── ERP.BuildingBlocks.Messaging/     # RabbitMQ MassTransit wrappers
│   │   ├── ERP.BuildingBlocks.Persistence/   # EF Core & Mongo base context
│   │   └── ERP.BuildingBlocks.Logging/       # Serilog & OpenTelemetry setup
│   │
│   ├── Services/
│   │   ├── Product/
│   │   │   ├── ProductService.API/           # Controllers, gRPC Endpoints, Middleware
│   │   │   ├── ProductService.Application/   # Commands, Queries, Handlers, DTOs
│   │   │   ├── ProductService.Domain/        # Aggregates, Entities, Value Objects, Events
│   │   │   └── ProductService.Infrastructure/  # EF Core, Mongo Repositories, gRPC Clients
│   │   │
│   │   ├── Inventory/
...
│   │
│   └── Gateway/
│       └── ERP.YarpGateway/                  # YARP Reverse Proxy Configuration
│
└── tests/
...
To avoid building an overwhelming collection of empty services, follow this phased execution plan:

```
┌────────────────────────────────────────────────────────────────────────┐
│ PHASE 1: Foundation & Gateway                                          │
│ Solution layout, YARP Gateway, Identity Service, Docker Compose setup  │
└───────────────────────────────────┬────────────────────────────────────┘
                                    │
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
